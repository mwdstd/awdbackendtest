...
def plot_hla(cnev, md, inc, az, hla_ref_dict, case_id):
    ...
    plt.figure()
    plt.plot(md, hla_calc[:, 0], 'r-', label='S_high calculated')
    plt.plot(md, hla_calc[:, 1], 'g-', label='S_lat calculated')
    plt.plot(md, hla_calc[:, 2], 'b-', label='S_along calculated')
    plt.plot(md, hla_ref[:, 0], 'r.', label='S_high benchmark')
    plt.plot(md, hla_ref[:, 1], 'g.', label='S_lat benchmark')
    plt.plot(md, hla_ref[:, 2], 'b.', label='S_along benchmark')
    plt.xlabel('MD, m')
...
def plot_osf(osf_calc, osf_bnch, case_id):
    ...
    plt.plot(osf_bnch[:, 0], osf_bnch[:, 1], 'go', label='Benchmark OSF')
    plt.xlabel('MD, m')
    plt.ylabel('OSF, -')
    plt.title(f'{case_id}')
    plt.legend()